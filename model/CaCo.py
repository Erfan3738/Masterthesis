...
class CaCo(nn.Module):
    ...
    def __init__(self, base_encoder,args, dim=128, m=0.999):
        ...
        # we do not keep 
        self.encoder_q.fc = nn.Sequential(nn.Linear(dim_mlp, 512), nn.ReLU(),nn.Linear(512, 128))
        self.encoder_k.fc = nn.Sequential(nn.Linear(dim_mlp, 512), nn.ReLU(),nn.Linear(512, 128))
